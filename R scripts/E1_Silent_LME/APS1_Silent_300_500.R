...
# R script--Analyze the APS data in E1 silent reading for N400 window
...
N2_modelmiddle2<-lmer(scale(value)~chlabel*grammar*word_type+scale(log(comprehension_score))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+chlabel*grammar*word_type|ERPset),data=VerbPro_silent_Middle)
summary(N2_modelmiddle2) #Cant converge, three way interaction included in the random structure
N2_modelmiddle3<-lmer(scale(value)~chlabel*grammar*word_type+scale(log(comprehension_score))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+chlabel*grammar*word_type|ERPset),data=VerbPro_silent_Middle)
summary(N2_modelmiddle3) # can't converge
N2_modelmiddle3<-lmer(scale(value)~chlabel*grammar*word_type+scale(log(comprehension_score))+scale(log(reading_speed))+(1+chlabel*grammar+chlabel*word_type|ERPset),data=VerbPro_silent_Middle)
summary(N2_modelmiddle3) #can't converge
N2_modelmiddle4<-lmer(scale(value)~chlabel*grammar*word_type+scale(log(comprehension_score))+scale(log(reading_speed))+(1+chlabel*grammar+chlabel*word_type||ERPset),data=VerbPro_silent_Middle)
summary(N2_modelmiddle4) #can't converge

N2_modelmiddle5<-lmer(scale(value)~chlabel*grammar*word_type+scale(log(comprehension_score))+scale(log(reading_speed))+(1+grammar+chlabel+word_type||ERPset),data=VerbPro_silent_Middle)
summary(N2_modelmiddle5) ## BEST MODEL for MIDLINE Analysis
...
#LMM resutls for middleline for Subject_Verb Disagreement
Verb_silent_Middle$chlabel<-relevel(Verb_silent_Middle$chlabel,ref="FZ")
N2_modelmiddle2<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+scale(log(Accuracy))+scale(log(comprehension_score))+grammar+chlabel|ERPset), data=Verb_silent_Middle)
summary(N2_modelmiddle2) # Full model fail to converge
N2_modelmiddle3<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))+grammar*scale(log(reading_speed))+(1+grammar+chlabel|ERPset),data=Verb_silent_Middle)
summary(N2_modelmiddle3) # no significant interaction between offline speed and grammaticalality 

N2_modelmiddle3b<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+grammar+chlabel|ERPset),data=Verb_silent_Middle)
summary(N2_modelmiddle3b) #Best Model for subject-verb disagreement midline N400 analysis

anova(N2_modelmiddle3,N2_modelmiddle3b)
...
#########################################################################
#LMM resutls for middleline for Pronoun Mismatch
summary(Pronoun_silent_Middle$grammar)
Pronoun_silent_Middle$chlabel<-relevel(Pronoun_silent_Middle$chlabel,ref="FZ")

Pronoun_modelmiddle1<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+grammar+chlabel|ERPset),data=Pronoun_silent_Middle)
summary(Pronoun_modelmiddle1) #fit
Pronoun_modelmiddle2<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(comprehension_score))+grammar*scale(log(reading_speed))+(1+grammar+chlabel|ERPset),data=Pronoun_silent_Middle)
summary(Pronoun_modelmiddle2) #fit
anova(Pronoun_modelmiddle1,Pronoun_modelmiddle2)
Pronoun_modelmiddle3<-lmer(scale(value)~grammar*chlabel+scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))+(1+grammar+chlabel++scale(log(Accuracy))+scale(log(reading_speed))+scale(log(comprehension_score))||ERPset),data=Pronoun_silent_Middle)
summary(Pronoun_modelmiddle3) #Best Model


############# NON-MIDLINE ANALYSIS #############################
#LMM subset the channels for verb and Pronoun Lateral analysis
Silent_win300_500$chlabel<-str_trim(Silent_win300_500$chlabel) #Delete the space before and after the targe words.
table(Silent_win300_500$chlabel)
...
N400_Verb_silent_model2b<-lmer(scale(value)~grammar*word_type*position1+grammar*word_type*position2+scale(comprehension_score)+scale(reading_speed)+(1+grammar*word_type+position1+position2|ERPset),data=Verb_silent)
summary(N400_Verb_silent_model2b) #More complex model with three way interaction. What can we know?

N400_Verb_silent_model2c<-lmer(scale(value)~grammar*word_type*position1*position2+scale(reading_speed)+scale(comprehension_score)+(1+grammar*word_type+position1+position2|ERPset),data=Verb_silent)
summary(N400_Verb_silent_model2c) # With all the interactions
...
summary(N400_V_silent_model2) 
N400_V_silent_model4<-lmer(scale(value)~grammar+grammar*position2+grammar*position1+position1+position2+scale(Accuracy)+grammar*scale(reading_speed)+grammar*scale(comprehension_score)+(1+grammar+position1+position2|ERPset),data=V_silent)
summary(N400_V_silent_model4) #No interaction between grammar and OFFLINE comprehension score
N400_V_silent_model5<-lmer(scale(value)~grammar+grammar*position2+grammar*position1+position1+position2+grammar*scale(Accuracy)+grammar*scale(reading_speed)+scale(comprehension_score)+(1+grammar+position1+position2|ERPset),data=V_silent)
summary(N400_V_silent_model5) #No interation between grammar and online Accuracy 
...
# the interaction between grammar and reading speed was no significant 
N400_P_silent_model4<-lmer(scale(value)~grammar+grammar*position2+grammar*position1+position1+position2+scale(Accuracy)+scale(reading_speed)+scale(comprehension_score)+(1+grammar+position1+position2|ERPset),data=P_silent)
summary(N400_P_silent_model4) #Best grammar and positions interaction,  grammar and offline reading speed interaction
