...
# when they imagining the Non-native Chinese-accented English speaker's voice

setwd("Set working directory here")
cond1<-read.csv("APS2_Bin3_Chinese Correct Verb in Good Pronoun.csv",sep=",", comment.char ="", quote='"', na.strings ="", header=T)
cond3<-read.csv("APS2_Bin7_Chinese Good Pronoun in correct Verb.csv",sep=",", comment.char ="", quote='"', na.strings ="", header=T)
cond4<-read.csv("APS2_Bin8_Chinese Bad Pronoun in Correct Verb .csv",sep=",", comment.char ="", quote='"', na.strings ="", header=T)
...
#cond2_type <- 5				#What type of line do you want cond2's line to be?
...
start <- match(presample, cond3$time)		#Finds the row number of the starting point
stop <- match(epoch, cond3$time)			#Finds the row number of the end point
base1 <- match(base_begin, cond3$time)		#Finds the row number of the beginning of the baseline
base2 <- match(base_end, cond3$time)		#Finds the row number of the end of the baseline
base3 <- match(base_begin, cond3$time)		#Finds the row number of the beginning of the baseline
base4 <- match(base_end, cond3$time)		#Finds the row number of the end of the baseline
...
##############################################################################################################
#### Rebaseline and plot F3 cond3.
############################################################################################################## 
#base_mean_cond3 <- mean(cond3$F3[base1:base2])     #Calculate mean amplitude in baseline interval
#rebase_cond3 <- cond3$F3 - base_mean_cond3         #Re-baseline that condition
...
##############################################################################################################
#### Rebaseline and plot FZ cond3.
############################################################################################################## 
#base_mean_cond3 <- mean(cond3$FZ[base1:base2])     #Calculate mean amplitude in baseline interval
#rebase_cond3 <- cond3$FZ - base_mean_cond3         #Re-baseline that condition
#lines(cond3$time[start:stop], rebase_cond3[start:stop], type="l",lwd=line_width, lty=cond3_type, col=cond3_color)   #Plot it
...
# Save plot as pdf
dev.copy(pdf, "APS2 Pronoun Chinese_dashline.pdf", width=7.5, height=9)
dev.off()
